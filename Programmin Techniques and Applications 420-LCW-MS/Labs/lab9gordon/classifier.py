'''
This is a set of generic functions and classes that are useful with 
all of the supervised learning classifiers.
'''
class data_item(object):
    ...
    def __init__(self, label, data):
        ...
        self.data = data
    def __repr__(self):
        return 'data_item(' + str(self.label) + ', ' + str(self.data) + ')'
...
def distance(d1, d2):
    '''n-dimensional Euclidean distance between 'd1' and 'd2'.'''
...
class classifier(object):
    ...
    This is an example of an 'abstract class', in the sense that it
    is not itself a working implementation, but rather just a generic
    class that defines the key methods, train() and predict(), that
    are used by all of the supervised learning classifiers.
    '''
    def __init__(self):
